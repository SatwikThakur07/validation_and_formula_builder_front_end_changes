...
/**
 * GET /api/node/reconciliation/tenderList
 * Get list of distinct tenders (data_source values) from customised_db_fields table
 */
const getTenderList = async (req, res) => {
  try {
    console.log("[TENDER_LIST] Controller hit: GET /api/node/reconciliation/tenderList");
    
    // Query distinct data_source values from customised_db_fields table
    // Use raw query for DISTINCT
    const tenders = await db.bercos.query(`
    ...
    // Extract data_source values and format them
    const rawTenderList = tenders
      .map(row => row.data_source)
      .filter(Boolean);
    
    // Format tender names: replace _ with space, title case, but keep POS uppercase
    const formatTenderName = (name) => {
      const replaced = name.replace(/_/g, ' ');
      const words = replaced.split(' ');
      return words.map(word => {
        if (word.toUpperCase() === 'POS') {
          return 'POS';
        }
    ...
    const tenderList = rawTenderList.map(formatTenderName);
    
    console.log("[TENDER_LIST] Response:", JSON.stringify(tenderList, null, 2));
    
    res.status(200).json(tenderList);
  } catch (error) {
    console.error("[TENDER_LIST] Error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching tender list",
      error: error.message
    });
  }
};
...
/**
 * POST /api/node/reconciliation/tenderWisetables
 * Get tender-wise tables and columns from customised_db_fields table
 */
const getTenderWiseTables = async (req, res) => {
  try {
    console.log("[TENDER_WISE_TABLES] Controller hit: POST /api/node/reconciliation/tenderWisetables");
    const { tenders } = req.body;
    
    console.log("[TENDER_WISE_TABLES] Request tenders:", tenders);
    
    // Normalize tenders to array
    let tenderList = [];
    if (Array.isArray(tenders)) {
      tenderList = tenders.map(t => t.trim());
    } else if (typeof tenders === 'string') {
      tenderList = tenders.split(',').map(t => t.trim());
    }
    ...
    // Format tender names for database query (replace spaces with underscores, uppercase)
    const formatTenderForDb = (tender) => {
      return tender.replace(/\s+/g, '_').toUpperCase();
    };
    
    const dbTenderList = tenderList.map(formatTenderForDb);
    console.log("[TENDER_WISE_TABLES] Formatted tenders for DB:", dbTenderList);
    
    // Query customised_db_fields table
    const records = await db.customised_db_fields.findAll({
      where: {
        tender_name: {
          [Op.in]: dbTenderList
        },
      ...
      order: [['tender_name', 'ASC'], ['data_source', 'ASC'], ['excel_column_name', 'ASC']],
      raw: true
    });
    
    console.log("[TENDER_WISE_TABLES] Found", records.length, "records");
    
    // Group data by tender -> dataSource -> columns
    const tenderDataMap = {};
    
    records.forEach(row => {
      const tender = row.tender_name;
      const dataSource = row.data_source;
      
      if (!tenderDataMap[tender]) {
        tenderDataMap[tender] = {};
      }
      
      if (!tenderDataMap[tender][dataSource]) {
        tenderDataMap[tender][dataSource] = {
      ...
      const columnExists = tenderDataMap[tender][dataSource].columns.some(
        col => col.excelColumnName === row.excel_column_name
      );
      
      if (!columnExists) {
        tenderDataMap[tender][dataSource].columns.push({
          excelColumnName: row.excel_column_name,
          dbColumnName: row.db_column_name || row.excel_column_name,
          columnName: row.db_column_name || row.excel_column_name
        });
      }
    });
    
    // Convert to response format
    const result = Object.keys(tenderDataMap).map(tender => {
      const dataSourceWiseColumns = Object.values(tenderDataMap[tender]);
      return {
        tender: tender,
        dataSourceWiseColumns: dataSourceWiseColumns
      };
    });
    
    console.log("[TENDER_WISE_TABLES] Response:", JSON.stringify(result, null, 2));
    ...
    console.error("[TENDER_WISE_TABLES] Error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching tender-wise tables",
      error: error.message
    });
  }
};
...
/**
 * POST /api/node/reconciliation/recologics/save
 * Save recologics formulas to the reco_logics table
 */
const saveRecologics = async (req, res) => {
  try {
    console.log("[RECOLOGICS] Save endpoint hit");
    const { tenders, recoData, effectiveFrom, effectiveTo, effectiveType, id } = req.body;
    
    console.log("[RECOLOGICS] Tenders:", tenders);
    console.log("[RECOLOGICS] Formulas count:", recoData?.length || 0);
    console.log("[RECOLOGICS] Has ID (update?):", id !== null && id !== undefined);
    
    // Normalize tenders to comma-separated string
    let tenderStr = '';
    if (Array.isArray(tenders)) {
      if (tenders.length === 0) {
        return res.status(400).json({
      ...
      tenderStr = tenders.trim();
    } else {
      return res.status(400).json({
        success: false,
        message: "Invalid tenders format"
      });
    }
    ...
    if (!recoData || !Array.isArray(recoData) || recoData.length === 0) {
      return res.status(400).json({
        success: false,
        message: "At least one formula (recoData) is required"
      });
    }
    
    // Convert recoData to JSON string for storage
    const recologicJson = JSON.stringify(recoData, null, 2);
    ...
    // Handle effectiveTo - default to 2099-12-31 if not provided
    const effectiveToDate = effectiveTo || '2099-12-31';
    
    // If ID is provided, update existing record
    if (id !== null && id !== undefined) {
      const existingRecord = await db.reco_logics.findByPk(id);
      ...
      await db.reco_logics.update({
        updated_date: currentTimestamp,
        recologic: recologicJson,
        tender: tenderStr,
        effectivefrom: effectiveFrom,
        effectiveto: effectiveToDate,
        effectivetype: effectiveType || 'business_date',
        status: 'UPDATED'
      }, {
        where: { id: id }
      });
      
      console.log("[RECOLOGICS] Updated record with ID:", id);
      ...
      // Insert new record
      const newRecord = await db.reco_logics.create({
      ...
      console.log("[RECOLOGICS] Created new record with ID:", newRecord.id);
    ...
    console.error("[RECOLOGICS] Save error:", error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/node/reconciliation/recologics/getAll
 * Get all saved recologics formulas, optionally filtered by tenders
 */
const getAllRecologics = async (req, res) => {
  try {
    console.log("[RECOLOGICS] GetAll endpoint hit");
    const { tenders } = req.query;
    ...
    if (tenders) {
      // Normalize tenders to array
      let tenderList = [];
      if (typeof tenders === 'string') {
        tenderList = tenders.split(',').map(t => t.trim());
      } else if (Array.isArray(tenders)) {
        tenderList = tenders.map(t => t.trim());
      }
      
      if (tenderList.length > 0) {
        // Format for database (comma-separated string matching)
        whereClause.tender = {
          [Op.or]: tenderList.map(tender => ({
            [Op.like]: `%${tender}%`
          }))
        };
      }
    ...
    const records = await db.reco_logics.findAll({
      where: whereClause,
      order: [['created_date', 'DESC']],
      raw: true
    });
    
    // Parse recologic JSON and format response
    const formattedRecords = records.map(record => {
      let recoData = [];
      try {
        if (record.recologic) {
          recoData = JSON.parse(record.recologic);
        }
      } catch (e) {
        console.error("[RECOLOGICS] Error parsing recologic JSON:", e);
      }
      
      return {
        id: record.id,
        tenders: record.tender ? record.tender.split(',') : [],
        recoData: recoData,
    ...
    console.log("[RECOLOGICS] GetAll response:", formattedRecords.length, "records");
    ...
    console.error("[RECOLOGICS] GetAll error:", error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/node/reconciliation/recologics/get
 * Get recologics formulas by topic/tender
 */
const getRecologicsByTopic = async (req, res) => {
  try {
    console.log("[RECOLOGICS] GetByTopic endpoint hit");
    const { tenders, topic } = req.query;
    ...
    // Parse and filter by topic if provided
    const formattedRecords = records.map(record => {
      let recoData = [];
      try {
        if (record.recologic) {
      ...
      if (topic && recoData.length > 0) {
        recoData = recoData.filter(item => 
          item.topic === topic || item.logicName === topic || item.dataset === topic
        );
      }
    ...
    }).filter(record => record.recoData.length > 0); // Only return records with matching data
    
    console.log("[RECOLOGICS] GetByTopic response:", formattedRecords.length, "records");
    ...
    console.error("[RECOLOGICS] GetByTopic error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching formulas by topic",
      error: error.message
    });
  }
};
...
/**
 * GET /api/node/reconciliation/datasource
 * Get datasource for mapping (similar to uploader datasource endpoint)
 */
const getDatasourceForMapping = async (req, res) => {
  try {
    console.log("[DATASOURCE] GetDatasourceForMapping endpoint hit");
    
    // Query data_source table using bercos database (main database)
    // Note: If data_source table doesn't exist, we can query from customised_db_fields instead
    let dataSources = [];
    try {
      dataSources = await db.bercos.query(`
        SELECT 
          category,
          tender,
          type,
      ...
      // If data_source table doesn't exist, fallback to customised_db_fields
      console.log("[DATASOURCE] data_source table not found, using customised_db_fields");
      const fallbackData = await db.customised_db_fields.findAll({
        attributes: [
          'data_source',
          'tender_name',
          'table_name'
        ...
        where: {
          data_source: { [Op.ne]: null },
          tender_name: { [Op.ne]: null }
        },
        group: ['data_source', 'tender_name', 'table_name'],
        raw: true
      });
      
      // Transform to match expected format
      dataSources = fallbackData.map(row => ({
        category: 'Custom',
        tender: row.tender_name || 'Unknown',
        type: 'default',
        dataSource: row.data_source
      }));
    }
    
    console.log("[DATASOURCE] Found", dataSources.length, "records");
    ...
    dataSources.forEach(row => {
      ...
      if (!categoryMap[category][tender]) {
        categoryMap[category][tender] = {};
      }
      
      categoryMap[category][tender][dataSource] = type;
    });
    
    // Convert to response format
    const result = [];
    const categoryOrder = ["3PO", "Banking", "POS", "Config", "Cash_PickUp"];
    
    categoryOrder.forEach(category => {
      if (categoryMap[category] && Object.keys(categoryMap[category]).length > 0) {
        const tenders = [];
        Object.keys(categoryMap[category]).sort().forEach(tender => {
          const dataSourceMap = categoryMap[category][tender];
          const types = Object.keys(dataSourceMap).sort().map(dataSource => ({
            type: dataSourceMap[dataSource],
    ...
    console.log("[DATASOURCE] Response:", result.length, "categories");
    ...
    console.error("[DATASOURCE] GetDatasourceForMapping error:", error);
    res.status(500).json({
      success: false,
      message: "Error fetching datasource",
      error: error.message
    });
  }
};
...
module.exports = {
  ...
  saveRecologics,
  getAllRecologics,
  getRecologicsByTopic,
  getDatasourceForMapping,
};
