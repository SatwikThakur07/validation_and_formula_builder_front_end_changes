...
# ============================================================================
# RECOLOGICS ENDPOINTS - Compatible with vikas_sir_config
# ============================================================================

class SaveRecoLogicsRequest(BaseModel):
    """Request model for saving recologics formulas - matches vikas_sir_config format"""
    tenders: Union[str, List[str]] = Field(..., description="Tender names (string or list)")
    recoData: List[Dict[str, Any]] = Field(..., description="Array of formula objects")
    effectiveFrom: str = Field(..., description="Effective from date (YYYY-MM-DD)")
    effectiveTo: Optional[str] = Field(default="2099-12-31", description="Effective to date (YYYY-MM-DD)")
    effectiveType: str = Field(default="business_date", description="Effective type")
    id: Optional[int] = Field(None, description="Optional ID for updates (if provided, will update instead of insert)")
...
async def save_recologics(
    ...
    Stores the recoData array as JSON in the recologic column.
    Compatible with vikas_sir_config format.
    
    Request format (matches vikas_sir_config):
    {
        "tenders": ["ZOMATO", "POS Orders"] or "ZOMATO,POS Orders",
    ...
    """
    try:
        logger.info("[RECOLOGICS] Save endpoint hit")
        logger.info(f"[RECOLOGICS] Tenders: {request_data.tenders}")
        logger.info(f"[RECOLOGICS] Formulas count: {len(request_data.recoData)}")
        logger.info(f"[RECOLOGICS] Has ID (update?): {request_data.id is not None}")
        
        # Normalize tenders to comma-separated string (handles both list and string)
        if isinstance(request_data.tenders, list):
            if len(request_data.tenders) == 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="At least one tender is required"
                )
            tender_str = ",".join(request_data.tenders)
        else:
            if not request_data.tenders or request_data.tenders.strip() == "":
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="At least one tender is required"
                )
            tender_str = request_data.tenders.strip()
        
        # Validate recoData
        if not request_data.recoData or len(request_data.recoData) == 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="At least one formula (recoData) is required"
            )
        
        # Convert recoData to JSON string for storage in recologic column
        # Use indent=2 for readability (matches vikas_sir_config format)
        recologic_json = json.dumps(request_data.recoData, ensure_ascii=False, indent=2)
        
        # Get current timestamp
        current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        # Get username for createdby field
        created_by = current_user.username if current_user and current_user.username else "ADMIN"
        
        # Handle effectiveTo - default to 2099-12-31 if not provided
        effective_to = request_data.effectiveTo or "2099-12-31"
        
        # If ID is provided, update existing record (for compatibility with vikas_sir_config)
        if request_data.id is not None:
            ...
            if not existing:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"Recologics record with id {request_data.id} not found"
                )
            ...
            await db.execute(update_query, {
                ...
                "status": "UPDATED",
                "tender": tender_str,
                "effectivefrom": request_data.effectiveFrom,
            ...
            logger.info(f"[RECOLOGICS] Successfully updated formula with ID: {request_data.id}")
            
            return {
                "success": True,
                "code": 200,
                "message": f"Formula updated successfully with {len(request_data.recoData)} formula(s)",
                "data": {
                    "id": request_data.id,
                    "tenders": tender_str.split(",") if "," in tender_str else [tender_str],
                    "formula_count": len(request_data.recoData),
                    "effective_from": request_data.effectiveFrom,
            ...
            insert_query = text("""
                ...
                (created_date, updated_date, recologic, remarks, status, tender, 
                 createdby, effectivefrom, effectiveto, effectivetype)
                VALUES 
                (:created_date, :updated_date, :recologic, :remarks, :status, :tender,
                 :createdby, :effectivefrom, :effectiveto, :effectivetype)
            """)
            ...
            params = {
                ...
                "updated_date": current_timestamp,
                "recologic": recologic_json,
                "remarks": None,  # Match vikas_sir_config format (no remarks)
                "status": "UPDATED",
                "tender": tender_str,
            ...
            logger.info(f"[RECOLOGICS] Successfully saved formula with ID: {inserted_id}")
            
            # Return format matches vikas_sir_config expected response
            return {
                "success": True,
                "code": 200,
                "message": f"Formula saved successfully with {len(request_data.recoData)} formula(s)",
                "data": {
                    "id": inserted_id,
                    "tenders": tender_str.split(",") if "," in tender_str else [tender_str],
                    "formula_count": len(request_data.recoData),
        ...
        logger.error(f"[RECOLOGICS] Error saving formula: {e}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error saving formula: {str(e)}"
        )
