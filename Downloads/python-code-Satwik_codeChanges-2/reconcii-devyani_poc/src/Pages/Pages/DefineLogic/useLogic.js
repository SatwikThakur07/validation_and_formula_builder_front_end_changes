...
const useLogic = () => {
  ...
  const getSavedFormulas = async (tenderList) => {
    try {
      ...
      if (response.status && response.data?.data?.length > 0) {
        try {
          ...
          response.data?.data?.forEach((group) => {
            let logicJson = [];
            try {
              // recologic is already parsed JSON from backend, but handle both cases
              if (typeof group?.recologic === 'string') {
                logicJson = JSON.parse(group?.recologic);
              } else if (Array.isArray(group?.recologic)) {
                logicJson = group?.recologic;
              } else {
                logicJson = [];
              }
            } catch (e) {
              console.error("[GET_SAVED_FORMULAS] Error parsing recologic:", e);
              logicJson = [];
            }
            let ele = {
              ...group,
          ...
          // Get the first group's formulas
          let firstGroup = response.data?.data[0];
          let savedLogic = [];
          try {
            if (typeof firstGroup?.recologic === 'string') {
              savedLogic = JSON.parse(firstGroup?.recologic);
            } else if (Array.isArray(firstGroup?.recologic)) {
              savedLogic = firstGroup?.recologic;
            }
          } catch (e) {
            console.error("[GET_SAVED_FORMULAS] Error parsing first group recologic:", e);
            savedLogic = [];
          }
          dispatch(setLogicData(savedLogic));
          dispatch(setDBLogicId(firstGroup?.id));
        } catch (e) {
          console.error("[GET_SAVED_FORMULAS] Error processing response:", e);
        ...
        dispatch(
          setLogicGroups([
            {
              tender: Array.isArray(tenderList) ? tenderList.join(",") : tenderList,
              createdBy: "ADMIN",
      ...
      console.error("[GET_SAVED_FORMULAS] Error:", error);
      dispatch(setDBLogicId(null));
    } finally {
      setLoading(false);
    }
