...
const useDashboard = () => {
  ...
  const downloadAsyncReport = async (params) => {
    ...
    if (params?.reportType === "POSVsThreePO") {
      ...
      return;
    } else if (params?.reportType === "POSVsTRM") {
      try {
        setLoading(true);
        const response = await requestCallPost(
          apiEndpoints.DOWNLOAD_POS_VS_TRM_DATA,
          currentDashboardRequest,
          {},
          { responseType: "blob" }
        );

        const blob = response.data; // Axios puts the blob in `data`
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement("a");

        const fileName = `pos_vs_trm_P014.xlsx`;
      ...
      return;
    } else if (params?.reportType === "TRMVsMPR") {
      try {
        setLoading(true);
        const response = await requestCallPost(
          apiEndpoints.DOWNLOAD_TRM_VS_MPR_DATA,
          currentDashboardRequest,
          {},
          { responseType: "blob" }
        );
        ...
        const fileName = `trm_vs_mpr_P014.xlsx`;
      ...
      return;
    } else if (params?.reportType === "MPRVsBank") {
      try {
        setLoading(true);
        const response = await requestCallPost(
          apiEndpoints.DOWNLOAD_MPR_VS_BANK_DATA,
          currentDashboardRequest,
          {},
          { responseType: "blob" }
        );
        ...
        const fileName = `mpr_vs_bank_P014.xlsx`;
